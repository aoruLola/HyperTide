...
> 本仓库是 HyperTide 商业孵化阶段的私有主代码库。目前不作为开源项目发布，也不授予开源许可。
>
> This repository is the private primary codebase for the commercial incubation phase of HyperTide. It is not published as an open source project, and no open source license is granted at this stage.
...
安全敏感问题应按 [SECURITY.md](SECURITY.md) 中的私有报告路径处理，不应公开披露。仓库当前处于商业孵化阶段，未授权开源使用、再发布或外部分发。

Security-sensitive findings should follow the private reporting path in [SECURITY.md](SECURITY.md) and should not be disclosed publicly. This repository is in commercial incubation and does not grant open source use, redistribution, or external publication rights.
